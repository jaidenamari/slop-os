---
description: Load documentation from their repsective websites into local markdown files our agents can use as context.
allowed-tools: Task, WebFetch, Write, Edit, Bash(ls*), mcp__firecrawl-mcp__firecrawl_scrape
---

# Load AI docs

Load documentation from their respective websites into local markdown files our agent can use as context.
...
2. See if any ai_docs/<some-filename>.md already exists
    1. If it does, see if it was created with the last `DELETED_OLD_AI_DOCS_AFTER_HOURS`
...
4. After all Tasks are complete, response in the `Report Format`
